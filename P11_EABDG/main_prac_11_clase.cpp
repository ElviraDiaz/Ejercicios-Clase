...
Instrucciones de uso:
	Con las teclas del teclado se puede mover el conjunto de modelos
	D: Mueve los modelos a la izquierda
	A: Mueve los modelos a la derecha
	W: Acerca los modelos
	S: Aleja los modelos

	Barra espaciadora: Hace que el auto se mueva
	R: Hace que el auto recorra el circuito yendo hacia la izquierda
	D: Hace que el auto recorra el circuito yendo hacia la derecha
	P: Se detiene el auto
...
void animate(void)
{
	if (play) {//¿Se debe mover?
		printf("x=%f, y=%f,Z=%f\n", movKit_x, movKit_y, movKit_z);
		if (reversa==false) {//Recorrido hacia la derecha
			if (estado == 0) {//Edo A

				if (movKit_z < 150) {//si sigo en el edo A
					movKit_z += 1.0f;
				}
				else {		//Cambio al edo B
					rotKit_y = 90;
					estado = 1;
				}
			}
			...
			if (estado == 1) {//Edo B
				...
				if (movKit_x < 180) {//Si sigo en el edo B
					movKit_x += 1.0f;
					rotKit_y = 90;
				}
				else {				//Cambio al edo C
					rotKit_y = 180;
					estado = 2;
				}
			}
			if (estado == 2) {//Edo C
				...
				if ( movKit_z > -147) {	//Si sigo en el edo C
					movKit_z -= 1.0f;
					rotKit_y = 180;
				}
				else {			//Cambio al edo D
					rotKit_y = 270;
					estado = 3;
				}
			}
			if (estado == 3) {//Edo D
				...
				if (movKit_x > 0) {	//Si sigo en el edo D
					movKit_x -= 1.0f;
					rotKit_y = 270;
				}
				else {			//Cambio al edo A
					rotKit_y = 0;
					estado = 0;
				}
			}
		}
		if (reversa) {			//Recorrido a la izquierda
			if (estado == 0) {//Edo A
				if (movKit_z >-147) {//si sigo en el edo A
			...
			if (estado == 2) {//Edo C
				...
				if ( movKit_z <150) {	//Si sigo en el edo C
					movKit_z += 1.0f;
					rotKit_y = 0;
